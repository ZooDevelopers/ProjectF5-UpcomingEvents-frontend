...
<template>
  <div class="relative">
      ...
      <div class="swiper-wrapper">
        <swiper-slide 
          ...
          <div class="slide-content flex flex-col">
            <div class="p-5 bg-purple-600 max-w-[600px] rounded-[20px]">
              <span class="flex flex-row gap-5 text-grey">
                <p>{{ event.getDate() }}</p>
                <p>{{ event.getLocation() }}</p>
              </span>
              <h3 class="text-peach-500 text-3xl">{{ event.getTitle() }}</h3>
            </div>
          </div>
        </swiper-slide>
      </div>
    ...
    <button
      ...
      :class="[ 
        'absolute top-1/2 -translate-y-1/2 left-4 z-10 w-12 h-12 rounded-full flex items-center justify-center', isBeginning ? 'bg-peach-600/60 cursor-not-allowed text-purple-700/50' : 'bg-peach-600 text-purple-700'
      ]"
    >
    ...
    <button
      ...
      :class="[
        'absolute top-1/2 -translate-y-1/2 right-4 z-10 w-12 h-12 rounded-full flex items-center justify-center', isEnd ? 'bg-peach-600/60 cursor-not-allowed text-purple-700/50' : 'bg-peach-600 text-purple-700' 
      ]"
    >
